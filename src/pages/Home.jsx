...
const Home = () => {
  ...
  const openCamera = async () => {
    ...
    if (!window.isSecureContext) {
      setFileError("Camera access requires HTTPS. Please access the site securely.");
      return;
    }

    if (!isCameraAvailable) {
      setFileError("Camera not supported on this device. Please upload images manually.");
      return;
    }
    ...
    try {
      ...
      setFileError(
        err.name === "NotAllowedError" ? "Camera access denied. Please grant permissions." :
        err.name === "NotFoundError" ? "No camera found. Please upload images manually." :
      ...
      {showCamera && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
            <h2 className="text-xl font-bold text-yellow-600 mb-4 text-center">Capture Image</h2>
